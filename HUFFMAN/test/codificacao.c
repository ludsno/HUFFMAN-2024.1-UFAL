...
// Função para criar o nome do arquivo compactado e descompactado
void criarNomeArquivo(const char *caminhoArquivo, char *nomeArquivo)
{
...
int compactarArquivo()
{
    ...
    criarNomeArquivo(caminhoArquivo, nomeArquivo);
...
int descompactarArquivo()
{
   ...
   for (int k = 0; k < tamArvore; k++)
        printf("%c", arvore[k]);  caso queira ver a arvore em pre-ordem, dxe esse for
   ...
   // imprimirArvoreHuffmanPO(arvoreHuffman);
    ...
    strcpy(temp, "descompactado");
    strcat(temp, nomeArquivo);
    strcpy(nomeArquivo, temp);
